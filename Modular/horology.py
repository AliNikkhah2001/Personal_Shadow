...
from PyQt6.QtGui import QPainterPath, QColor, QPen, QBrush, QFont
...
def draw_clock_ticks_and_indices(p, radius):
    ...
    if indices != "None":
        p.save(); p.setFont(QFont("Arial", 11, QFont.Weight.Bold))
        for i in range(1, 13):
            angle = (i * 30 - 90) * np.pi / 180; x = (radius - 25) * np.cos(angle); y = (radius - 25) * np.sin(angle)
            if indices == "Baton":
