  ...
  switch (book.motif) {
    ...
    case "efficiency": {
      ...
      for (let x = 140; x < width - 100; x += 150) {
        ...
        for (let y = 520; y < height - 250; y += 130) {
          ...
          ctx.stroke();
          strokeLine(ctx, [[x + 10, y], [x + 84, y]], 3);
        }
      }
      strokeLine(ctx, [[120, height - 315], [width - 120, 420]], 9);
      break;
    }
    case "network": {
      ...
      for (let x = 0; x < cols; x += 1) {
        for (let y = 0; y < rows; y += 1) {
          ...
          const py = 390 + y * 105;
          if (x < cols - 1) {
            strokeLine(ctx, [[px, py], [px + 126 + Math.sin(y + 1) * 18, py + 105]], 2.5);
          }
          ctx.beginPath();
    ...
    case "organization": {
      ...
      levels.forEach((count, level) => {
        ...
        for (let i = 0; i < count; i += 1) {
          ...
          ctx.beginPath();
          ctx.roundRect(x - 44, y - 32, 88, 64, 12);
          ctx.strokeStyle = book.ink;
          ctx.lineWidth = 4;
          ctx.stroke();
          ctx.strokeStyle = book.accent;
          if (level < levels.length - 1) {
            strokeLine(ctx, [[x, y + 32], [x, y + 128]], 3);
          }
        }
      });
      break;
    ...
    case "branches": {
      ...
      for (let level = 0; level < 5; level += 1) {
        ...
        for (let index = 0; index < count; index += 1) {
          ...
          strokeLine(ctx, [[parentX, parentY], [x, y]], 4);
          ctx.beginPath();
    ...
    case "runner": {
      ...
      for (let frame = 0; frame < 7; frame += 1) {
        ...
        ctx.fill();
        strokeLine(ctx, [[x, y - 98], [x + 4, y - 28], [x - 38 + frame * 4, y + 42]], 9);
        strokeLine(ctx, [[x + 2, y - 70], [x + 52, y - 42]], 8);
        strokeLine(ctx, [[x + 4, y - 28], [x + 48, y + 28]], 9);
      }
      break;
    }
    case "gather": {
    ...
    case "maze": {
      ...
      ctx.strokeStyle = book.accent;
      strokeLine(ctx, [
        [95, height - 215], [310, height - 215],
        [310, 680], [720, 680],
        [720, 430], [width - 95, 430],
      ], 14);
      break;
    }
    case "fracture": {
    ...
    case "windows": {
      ...
      ctx.beginPath();
      ctx.roundRect(width * 0.49, height * 0.62, 24, 118, 8);
      ctx.fill();
      break;
    }
    case "steps": {
      ...
      strokeLine(ctx, [[110, height - 235], [width - 115, 420]], 8);
      break;
    }
  }
  ctx.restore();
