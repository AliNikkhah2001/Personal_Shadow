                                ...
                                <div key={g.id} className="flex flex-col p-3 bg-white/5 rounded border border-white/5 hover:bg-white/10 transition mb-2" style={{ borderLeft: `3px solid ${rootColor}` }}>
                                    ...
                                    {g.target_hours > 0 && (
                                        <div className="flex justify-between items-center mt-1">
                                            <div className="flex-grow h-1.5 bg-black/50 rounded-full overflow-hidden mr-3">
                                                <div className="h-full rounded-full" style={{ width: `${Math.min(100, (logged/g.target_hours)*100)}%`, backgroundColor: rootColor }}></div>
                                            </div>
                                            <span className="text-[9px] text-gray-400 font-mono">{rem.toFixed(1)}h rem</span>
                                        </div>
                                    )}
                                </div>
                            );
                                ...
                                <div className="flex flex-wrap gap-1 flex-grow">
                                    {checkin.mood_tags.map((m, i) => (
                                        <span key={i} className="bg-purple-600/30 text-purple-300 px-2 py-0.5 rounded-full text-[9px] font-bold flex items-center gap-1">
                                            {m}
                                            <button onClick={() => setCheckin(prev => ({...prev, mood_tags: prev.mood_tags.filter((_, idx) => idx !== i))})} className="text-purple-500 hover:text-purple-300"><i className="fas fa-times text-[8px]"></i></button>
                                        </span>
                                    ))}
                                </div>
                            </div>
...
const DashboardHealthWidget = ({ healthProfile, healthLogs }) => {
                ...
                if (chartRef.current && window.Chart) {
                    ...
                    let chartInstance = new window.Chart(ctx, {
                        ...
                        data: {
                            ...
                            datasets: [{
                                label: 'Proj. Weight (kg)',
                                data: projWeights.length > 0 ? projWeights : [healthProfile?.weight || 70],
                                borderColor: '#3b82f6', tension: 0.3, yAxisID: 'y'
                            }, {
                                type: 'bar',
                                label: 'Deficit (kcal)',
                                data: deficits.length > 0 ? deficits : [0],
                                backgroundColor: '#22c55e', yAxisID: 'y1'
                            }]
                        },
                        options: {
                            responsive: true, maintainAspectRatio: false,
                            scales: {
                                y: { type: 'linear', position: 'left', ticks: {color: 'gray'} },
                                y1: { type: 'linear', position: 'right', ticks: {color: 'gray'}, grid: {drawOnChartArea: false} },
                                x: { ticks: {color: 'gray'} }
                            },
                            plugins: { legend: { labels: {color: 'white', boxWidth: 12, font: {size: 10}} } }
                        }
                    });
        ...
        const DashboardView = ({ layout, setLayout, goals, isEditingLayout, setIsEditingLayout, clockFeed, heatmap, habits, habitLogs, metrics, backend, refreshGoals, healthProfile, healthLogs, studiedHours, courseColors }) => {
